...
本仓库包含 **Spring Boot 后端**（根目录）与 **Vue 3 前端**（目录 `frontend/`），前端通过 REST 调用后端（接口约定见 `docs/backend-api.md`）。前端启动说明见 `frontend/README.md`。
...
详细接口与字段约定以 `docs/backend-api.md` 为准；数据库脚本见 `docs/init.sql`。
