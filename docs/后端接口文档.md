# 悦食汇 · 后端接口文档（API Reference）

本文件为 **唯一权威** 的接口说明（原 `backend-api.md` 已合并至此）。实现以 `src/main/java/**/*Controller.java` 为准；若有出入以代码为准并回写本文档。
...
- **普通商家员工**（`roleCode` 为 `SHOP_MANAGER` / `STAFF` 等）：数据库 `employee.shop_id` 为其所属店。调用 URL 或请求体中带 **`shopId`** 的接口时，**仅能操作本店**；访问其它店铺数据返回 **`403`**。
- **平台超级管理员**（`roleCode = SUPER_ADMIN`，含测试账号 **`admin`**）：可管理**任意店铺**（后台代入驻、跨店查看数据）；用于区分「入驻商户」与「平台运维」。
- **订单接单 / 配送 / 完成**：按**订单上的 `shopId`** 校验，禁止操作其它店铺的订单。
- **平台代入驻**：由管理端调用 **`POST /api/admin/shops`** 创建店铺，再调用 **`POST /api/admin/shops/{shopId}/employees/bootstrap`** 创建首个店长账号；详情见本文 **§10**。

### 登录失败与 HTTP 401（前后端约定）

- **`POST /api/auth/user/login`**、**`/employee/login`**、**`/rider/login`**：账号或密码错误时，后端抛出未授权异常，响应 **HTTP 401**，body 内 `code` 亦为 **401**，`msg` 为人类可读原因（如「用户名或密码错误」）。
- 这与「已登录状态下 Token 失效」同样可能表现为 **401**，但**语义不同**。前端 Axios 拦截器应对 **`/auth/*/login` 请求**的 401 **不要**触发「清空 Token + 整页跳登录页」，否则登录页会刷新，用户看不到错误提示（实现见 `frontend/src/api/request.js` 中对登录路径的豁免）。
...
说明：`shopId` 仅 `type` 为 `EMPLOYEE` 时有值；用户 / 骑手为 `null`。
...
- **Query**：`shopId`（可选）
  - 普通员工：不传则统计**当前登录员工所属店铺**；若传 `shopId`，必须与本人店铺一致，否则 `403`。
  - `SUPER_ADMIN`：可传任意营业中店铺的 `shopId` 查看该店统计；不传则仍使用员工记录中的 `shopId`（如默认总店）。
...
说明：返回 `business_status = 1` 的店铺，供 C 端首页选店。无需登录（与 `DishController` 实现一致）。

返回每条店铺除基础字段外，可包含 **`categoryCount`、`dishCount`、`minPrice`、`featuredDishes`** 等聚合信息（便于列表展示推荐菜与规模），以前端实际解析为准。
...
- `GET /api/user/categories?shopId=1`
...
以下路径以 `CartController`（`/api/user/cart`）为准；`userId` 由 Token 解析，**无需** Query 传参。
...
- `PATCH /api/merchant/orders/{orderId}/accept?deliveryMode=SELF`

Query：

- `deliveryMode`：可选，默认 `SELF`
  - `SELF`：商家自配送，订单 **`已支付 → 已接单`**
  - `RIDER`：交由骑手池，**不改变**为「已接单」状态（订单仍在待骑手接单池，具体行为以 `OrderServiceImpl` 为准）
...
## 10. 管理端 Admin（需 SUPER_ADMIN）

Base：`/api/admin`。鉴权：员工 Token，且 `roleCode = SUPER_ADMIN`（与商家端同一登录入口 `/api/auth/employee/login`，使用管理员账号）。

管理后台前端页面「商家管理」中提供：**新建店铺**、**创建店长账号**（调用下列两个接口）。

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/admin/dashboard` | 平台统计看板 |
| GET | `/api/admin/users` | 用户分页列表 |
| PATCH | `/api/admin/users/{id}/toggle?enabled=` | 启用/禁用用户 |
| GET | `/api/admin/riders` | 骑手分页列表 |
| PATCH | `/api/admin/riders/{id}/toggle?enabled=` | 启用/禁用骑手 |
| GET | `/api/admin/shops` | 全部店铺列表 |
| POST | `/api/admin/shops` | **平台代入驻：新建店铺** |
| POST | `/api/admin/shops/{shopId}/employees/bootstrap` | **平台代入驻：为指定店创建首个员工（默认店长）** |
| PATCH | `/api/admin/shops/{id}/toggle?businessStatus=` | 店铺营业状态 |
| GET | `/api/admin/orders` | 全局订单（可选 `status`、`shopId`） |
...
说明：`shopName` 必填；`address`、`phone`、`notice` 可选；`businessStatus` 可选，`0` 打烊 / `1` 营业，默认 `1`。

返回 `data` 含新建店铺 `id`、`shopName`、`businessStatus` 等。

### 10.2 创建店长（或其它角色）账号

- `POST /api/admin/shops/{shopId}/employees/bootstrap`

请求体：

```json
{
  "username": "jiangnan_mgr",
  "password": "初始密码",
  "realName": "张店长",
  "phone": "13900000001",
  "roleId": 2
...
说明：`username`、`password` 必填；`realName`、`phone` 可选；`roleId` 可选，**默认 `2`（店长 `SHOP_MANAGER`）**。创建成功后，该账号使用 **`/api/auth/employee/login`** 登录**商家端**，仅能管理 **该 `shopId`** 下的业务（见本文开头「多店铺与商家权限」）。
...
- 商家端带 `shopId` 的读写接口按店铺隔离；非超级管理员不可跨店访问或篡改数据
- 店铺打烊时禁止下单
- 菜品下架或库存不足时禁止加入购物车
- 未支付订单才能取消
- 订单状态流转必须按顺序，不允许跳状态
